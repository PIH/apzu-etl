DROP PROCEDURE IF EXISTS create_last_tb_outcome_at_facility#
CREATE PROCEDURE `create_last_tb_outcome_at_facility`(IN _endDate DATE, IN _location VARCHAR(255))
BEGIN
	DROP TEMPORARY TABLE IF EXISTS last_tb_facility_outcome;

    CREATE TEMPORARY TABLE last_tb_facility_outcome(
	 ...
	 INSERT INTO last_tb_facility_outcome (index_desc, pat,identifier, state,program,program_state_id, start_date,end_date,location,location_key)
...
FROM (SELECT
      ...
      FROM omrs_program_state,
                    (SELECT @r:= 1) AS r,
                    (SELECT @u:= 0) AS u
                    where program = "Tuberculosis program"
            ...
            and opi.type = "TB program identifier"
