...
select
    ...
    max(sv.passage_normal) as passage_normal,
    max(sv.physiotherapy) as physiotherapy,
    max(sv.referred_out) as referred_out,
    max(sv.referred_out_specify) as referred_out_specify,
    max(sv.referred_to_poser) as referred_to_poser,
    max(sv.sleep_apnea) as sleep_apnea,
    max(sv.sleep_chocking) as sleep_chocking,
    max(sv.vomiting) as vomiting,
