...
select
    ...
    max(sv.dose_adjusted) as dose_adjusted,
    max(sv.dose_and_drug) as dose_and_drug,
    max(sv.feeding_cup) as feeding_cup,
    max(sv.feeding_sucking) as feeding_sucking,
    max(sv.feeding_tube) as feeding_tube,
    max(sv.feeding_counselling) as feeding_counselling,
    ...
    max(sv.group_counselling_and_play_therapy) as group_counselling_and_play_therapy,
    max(sv.referred_out) as referred_out,
    max(sv.referred_out_specify) as referred_out_specify,
    max(sv.referred_to_poser) as referred_to_poser,
    max(sv.signs_of_cerebral_palsy) as signs_of_cerebral_palsy,
    max(sv.tone_hyper) as tone_hyper,
    max(sv.tone_hypo) as tone_hypo,
    max(sv.tone_normal) as tone_normal,
    max(sv.under_mental_health_care_program) as under_mental_health_care_program,
