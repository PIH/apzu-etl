...
drop temporary table if exists temp_tuberculosis_drug_sensitivity_testing_non_coded;
create temporary table temp_tuberculosis_drug_sensitivity_testing_non_coded as select encounter_id, value_text from omrs_obs where concept = 'Tuberculosis drug sensitivity testing, non-coded';
alter table temp_tuberculosis_drug_sensitivity_testing_non_coded add index temp_tuberculosis_drug_sensitivity_testing_non_coded_encounter_idx (encounter_id);

drop temporary table if exists temp_date_started_pulmonary_rehabilitation_program_prp;
create temporary table temp_date_started_pulmonary_rehabilitation_program_prp as select encounter_id, value_date from omrs_obs where concept = 'Date started pulmonary rehabilitation program (PRP)';
alter table temp_date_started_pulmonary_rehabilitation_program_prp add index temp_date_started_pulmonary_rehabilitation_program_prp_encounter_idx (encounter_id);
...
alter table temp_tuberculosis_case_classification add index temp_tuberculosis_case_classification_encounter_idx (encounter_id);
...
alter table temp_directly_observed_treatment_option add index temp_directly_observed_treatment_option_encounter_idx (encounter_id);

drop temporary table if exists temp_patient_reported_current_tuberculosis_prophylaxis;
create temporary table temp_patient_reported_current_tuberculosis_prophylaxis as select encounter_id, value_coded from omrs_obs where concept = 'Patient reported current tuberculosis prophylaxis';
alter table temp_patient_reported_current_tuberculosis_prophylaxis add index temp_patient_reported_current_tuberculosis_prophylaxis_encounter_idx (encounter_id);
...
left join temp_tuberculosis_drug_sensitivity_testing_non_coded tuberculosis_drug_sensitivity_testing_non_coded on e.encounter_id = tuberculosis_drug_sensitivity_testing_non_coded.encounter_id
left join temp_date_started_pulmonary_rehabilitation_program_prp date_started_pulmonary_rehabilitation_program_prp on e.encounter_id = date_started_pulmonary_rehabilitation_program_prp.encounter_id
...
left join temp_patient_reported_current_tuberculosis_prophylaxis patient_reported_current_tuberculosis_prophylaxis on e.encounter_id = patient_reported_current_tuberculosis_prophylaxis.encounter_id
