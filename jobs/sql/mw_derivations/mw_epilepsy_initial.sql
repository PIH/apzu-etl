...
-- Single scan of omrs_obs for all EPILEPSY_INITIAL obs.
-- All single-concept temp tables and pivot step 1/2 queries derive from this.
...
-- Build temp_exposures in three steps to correctly pair each exposure type with its date via obs_group_id.
...
select obs_group_id, value_date as exposure_date
from temp_epilepsy_obs
where concept = 'Date of exposure'
  and obs_group_id in (select obs_group_id from temp_exposure_name_obs);
...
select n.encounter_id, n.exposure_name, d.exposure_date
from temp_exposure_name_obs n
left join temp_exposure_date_obs d on d.obs_group_id = n.obs_group_id;
alter table temp_exposures add index temp_exposures_encounter_idx (encounter_id);

-- Build temp_complications in three steps to correctly pair each complication with its date via obs_group_id.
drop temporary table if exists temp_complication_name_obs;
create temporary table temp_complication_name_obs as
select obs_group_id, encounter_id, value_coded as complication_name
from temp_epilepsy_obs
...
select obs_group_id, value_date as complication_date
from temp_epilepsy_obs
where concept = 'Date of complication'
  and obs_group_id in (select obs_group_id from temp_complication_name_obs);
...
select n.encounter_id, n.complication_name, d.complication_date
from temp_complication_name_obs n
left join temp_complication_date_obs d on d.obs_group_id = n.obs_group_id;
...
create temporary table temp_age_of_epilepsy_diagnosis as select encounter_id, value_numeric from temp_epilepsy_obs where concept = 'Age of epilepsy diagnosis';
...
create temporary table temp_alcohol_trigger as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Alcohol trigger';
...
create temporary table temp_date_antiretrovirals_started as select encounter_id, value_date from temp_epilepsy_obs where concept = 'Date antiretrovirals started';
...
create temporary table temp_family_history_of_behavioral_problems as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Family history of behavioral problems';
...
create temporary table temp_complications_at_birth as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Complications at birth';
...
create temporary table temp_delayed_milestones as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Delayed milestones';
...
create temporary table temp_highest_level_of_school_completed as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Highest level of school completed';
...
create temporary table temp_emotional_stress_anger_boredom as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Emotional stress, anger, boredom';
...
create temporary table temp_family_history_of_epilepsy as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Family history of epilepsy';
...
create temporary table temp_fever_trigger as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Fever trigger';
...
create temporary table temp_injury_trauma_surgery_of_head as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Injury, trauma, surgery of head';
...
create temporary table temp_history_of_seizure as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'History of seizure';
...
create temporary table temp_hiv_status as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'HIV status';
...
create temporary table temp_hiv_test_date as select encounter_id, value_date from temp_epilepsy_obs where concept = 'HIV test date';
...
create temporary table temp_civil_status as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Civil status';
...
create temporary table temp_medication_history_text as select encounter_id, value_text from temp_epilepsy_obs where concept = 'Medication history (text)';
...
create temporary table temp_menstruation_trigger as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Menstruation trigger';
...
create temporary table temp_family_history_of_mental_illness as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Family history of mental illness';
...
create temporary table temp_missed_medication_trigger as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Missed medication trigger';
...
create temporary table temp_month_of_onset as select encounter_id, value_numeric from temp_epilepsy_obs where concept = 'Month of onset';
...
create temporary table temp_neonatal_infection_cerebral_malaria_and_or as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Neonatal infection, Cerebral Malaria, and/or Meningitis';
...
create temporary table temp_main_activity as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Main activity';
...
create temporary table temp_post_ictal_disorientation as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Post-ictal disorientation';
...
create temporary table temp_post_ictal_drowsiness as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Post-ictal drowsiness';
...
create temporary table temp_post_ictal_headache as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Post-ictal headache';
...
create temporary table temp_post_ictal_hyperactivity as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Post-ictal hyperactivity';
...
create temporary table temp_post_ictal_memory_loss as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Post-ictal memory loss';
...
create temporary table temp_post_ictal_nausea as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Post-ictal nausea';
...
create temporary table temp_post_ictal_paralysis as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Post-ictal paralysis';
...
create temporary table temp_post_ictal_poor_concentration as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Post-ictal poor concentration';
...
create temporary table temp_post_ictal_poor_verbal_or_cognition as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Post-ictal poor verbal or cognition';
...
create temporary table temp_pre_ictal_warning as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Pre-ictal warning';
...
create temporary table temp_absence_seizures as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Absence seizures';
...
create temporary table temp_atonic_seizures as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Atonic seizures';
...
create temporary table temp_clonic_seizures as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Clonic seizures';
...
create temporary table temp_complex_partial_seizure as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Complex partial seizure';
...
create temporary table temp_myoclonic_seizures as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Myoclonic seizures';
...
create temporary table temp_simple_partial_seizure as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Simple partial seizure';
...
create temporary table temp_tonic_seizures as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Tonic seizures';
...
create temporary table temp_seizures_grandmal as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Seizures grandmal';
...
create temporary table temp_unclassified_epileptic_seizures as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Unclassified epileptic seizures';
...
create temporary table temp_sleep_deprivation_and_overtired as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Sleep deprivation and overtired';
...
create temporary table temp_sound_light_and_touch as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Sound, light, and touch';
...
create temporary table temp_vdrl as select encounter_id, value_coded from temp_epilepsy_obs where concept = 'Vdrl';
alter table temp_vdrl add index temp_vdrl_encounter_idx (encounter_id);

drop temporary table if exists temp_year_of_onset;
create temporary table temp_year_of_onset as select encounter_id, value_numeric from temp_epilepsy_obs where concept = 'Year of onset';
alter table temp_year_of_onset add index temp_year_of_onset_encounter_idx (encounter_id);

insert into mw_epilepsy_initial (patient_id, visit_date, location, age_at_onset, alcohol_date_of_exposure, alcohol_exposure, alcohol_trigger, art_start_date, behavioral_family_history, burn_complication, complications_at_birth, date_of_burn_complication, date_of_injury_complication, date_of_psychosis_complication, date_of_status_epilepticus_complication, delayed_milestones, drug_related_complication, drug_related_complication_date, education_level, emotional_stress_anger_boredom_trigger, epilepsy_family_history, fever_trigger, head_trauma_injury_surgery, history_of_seizure, hiv_status, hiv_test_date, injury_complication, marital_status, medication_history, menstruation_trigger, mental_health_family_history, missed_medication_trigger, month_of_onset, neonatal_infection_cerebral_malaria_meningitis, occupation, other_complication, other_complication_date, pig_date_of_exposure, pig_exposure, post_ictal_disorientation, post_ictal_drowsiness, post_ictal_headache, post_ictal_hyperactivity, post_ictal_memory_loss, post_ictal_nausea, post_ictal_paralysis, post_ictal_poor_concentration, post_ictal_poor_verbal_or_cognition, pre_ictal_warning, psychosis_complication, seizure_type_absence, seizure_type_atonic, seizure_type_clonic, seizure_type_complex, seizure_type_myoclonic, seizure_type_simple, seizure_type_tonic, seizure_type_tonic_clonic, seizure_type_unclassified, sleep_deprivation_and_overtired_trigger, smoking_date_of_exposure, smoking_exposure, sound_light_and_touch_trigger, status_epilepticus_complication, traditional_medicine_date_of_exposure, traditional_medicine_exposure, vdrl, year_of_onset)
select
    e.patient_id,
    date(e.encounter_date) as visit_date,
    e.location,
    max(age_of_epilepsy_diagnosis.value_numeric) as age_at_onset,
    max(case when exposures.exposure_name = 'Alcohol' then exposures.exposure_date end) as alcohol_date_of_exposure,
    max(case when exposures.exposure_name = 'Alcohol' then exposures.exposure_name end) as alcohol_exposure,
    ...
    max(case when complications.complication_name = 'Burn' then complications.complication_name end) as burn_complication,
    max(complications_at_birth.value_coded) as complications_at_birth,
    max(case when complications.complication_name = 'Burn' then complications.complication_date end) as date_of_burn_complication,
    max(case when complications.complication_name = 'Injury' then complications.complication_date end) as date_of_injury_complication,
    max(case when complications.complication_name = 'Psychosis' then complications.complication_date end) as date_of_psychosis_complication,
    max(case when complications.complication_name = 'Status epilepticus' then complications.complication_date end) as date_of_status_epilepticus_complication,
    max(delayed_milestones.value_coded) as delayed_milestones,
    max(case when complications.complication_name = 'Side effects of treatment' then complications.complication_name end) as drug_related_complication,
    max(case when complications.complication_name = 'Side effects of treatment' then complications.complication_date end) as drug_related_complication_date,
    ...
    max(case when complications.complication_name = 'Injury' then complications.complication_name end) as injury_complication,
    ...
    max(case when complications.complication_name = 'Other' then complications.complication_name end) as other_complication,
    max(case when complications.complication_name = 'Other' then complications.complication_date end) as other_complication_date,
    max(case when exposures.exposure_name = 'Pig' then exposures.exposure_date end) as pig_date_of_exposure,
    max(case when exposures.exposure_name = 'Pig' then exposures.exposure_name end) as pig_exposure,
    ...
    max(case when complications.complication_name = 'Psychosis' then complications.complication_name end) as psychosis_complication,
    ...
    max(case when exposures.exposure_name = 'Smoking' then exposures.exposure_date end) as smoking_date_of_exposure,
    max(case when exposures.exposure_name = 'Smoking' then exposures.exposure_name end) as smoking_exposure,
    max(sound_light_and_touch.value_coded) as sound_light_and_touch_trigger,
    max(case when complications.complication_name = 'Status epilepticus' then complications.complication_name end) as status_epilepticus_complication,
    max(case when exposures.exposure_name = 'Traditional medicine' then exposures.exposure_date end) as traditional_medicine_date_of_exposure,
    max(case when exposures.exposure_name = 'Traditional medicine' then exposures.exposure_name end) as traditional_medicine_exposure,
...
left join temp_complications complications on e.encounter_id = complications.encounter_id
