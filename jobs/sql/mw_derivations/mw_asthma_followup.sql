...
select
    ...
    e.location,
    max(sv.asthma_severity) as asthma_severity,
    max(sv.cooking_indoor) as cooking_indoor,
    max(sv.copd) as copd,
    max(sv.day_symptoms) as day_symptoms,
    max(sv.exacerbation_today) as exacerbation_today,
    max(sv.height) as height,
    max(sv.inhaler_use_frequency_daily) as inhaler_use_frequency_daily,
    max(sv.inhaler_use_frequency_monthly) as inhaler_use_frequency_monthly,
    max(sv.inhaler_use_frequency_weekly) as inhaler_use_frequency_weekly,
    max(sv.inhaler_use_frequency_yearly) as inhaler_use_frequency_yearly,
    max(sv.night_symptoms) as night_symptoms,
    max(sv.number_of_cigarettes_per_day) as number_of_cigarettes_per_day,
    max(sv.other_diagnosis) as other_diagnosis,
    max(sv.passive_smoking) as passive_smoking,
    max(sv.planned_visit) as planned_visit,
    max(sv.steroid_inhaler_daily) as steroid_inhaler_daily,
