...
select
    ...
    max(sv.aspirin) as aspirin,
    max(sv.bb_aten) as bb_aten,
    max(sv.bb_bis) as bb_bis,
    max(sv.bb_prop) as bb_prop,
    max(sv.benzathine) as benzathine,
    max(sv.bibasilar_crackles) as bibasilar_crackles,
    max(sv.bp_diastolic) as bp_diastolic,
    max(sv.bp_systolic) as bp_systolic,
    max(sv.ccb_aml) as ccb_aml,
    max(sv.ccb_nif) as ccb_nif,
    max(sv.concern_for_depression_or_anxiety) as concern_for_depression_or_anxiety,
    max(sv.diet_salt_or_fluid) as diet_salt_or_fluid,
    max(sv.diuretic_furo) as diuretic_furo,
    max(sv.diuretic_hctz) as diuretic_hctz,
    max(sv.diuretic_spiro) as diuretic_spiro,
    max(sv.dry_cough) as dry_cough,
    max(sv.dyspnea_on_exertion) as dyspnea_on_exertion,
    max(sv.fatigue) as fatigue,
    ...
    max(sv.jvp_elevated) as jvp_elevated,
    max(sv.mental_health_referral) as mental_health_referral,
    max(sv.next_appointment_date) as next_appointment_date,
    max(sv.nyha_stage) as nyha_stage,
    max(sv.oedema) as oedema,
    max(sv.orthopnea) as orthopnea,
    max(sv.other_medications) as other_medications,
    max(sv.palliative_referral) as palliative_referral,
    max(sv.request_chw_f_u) as request_chw_f_u,
    max(sv.salt_or_fluid_restricted) as salt_or_fluid_restricted,
    max(sv.spo2) as spo2,
    max(sv.tobacco) as tobacco,
    max(sv.took_medications_today) as took_medications_today,
    max(sv.volume_status) as volume_status,
