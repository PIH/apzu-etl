...
select
    ...
    max(sv.on_insulin) as on_insulin,
    max(sv.asthma_classification) as asthma_classification,
    max(sv.seizure_activity) as seizure_activity,
    max(sv.num_seizures) as num_seizures,
    max(sv.hba1c) as hba1c,
    max(sv.serum_glucose) as serum_glucose,
    ...
    max(sv.proteinuria) as proteinuria,
    max(sv.creatinine) as creatinine,
    max(sv.hiv_result) as hiv_result,
    max(sv.visual_acuity) as visual_acuity,
    max(sv.cv_risk) as cv_risk,
    max(sv.hospitalized_since_last_visit) as hospitalized_since_last_visit,
    max(sv.mental_status_exam) as mental_status_exam,
    mhd.mental_health_drugs,
    max(sv.mental_health_drug_side_effect) as mental_health_drug_side_effect,
    max(sv.mental_stable) as mental_stable,
    max(sv.fundoscopy) as fundoscopy
